...
export function PatentInput({ onSubmit, isLoading, onKeywordSearch }: PatentInputProps) {
  ...
  const isPatentNumber = (value: string): boolean => {
    ...
    return (
      trimmed.match(/^10-\d+/) !== null ||
      trimmed.match(/^KR\d+/i) !== null ||
    ...
    <div className="w-full max-w-2xl mx-auto animate-fade-up">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <Input
            type="text"
            placeholder="특허번호 또는 키워드를 입력하세요 (예: 10-1234567 또는 제빵)"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            className="pl-12 pr-4 py-6 text-lg bg-card border-border/50 focus:border-accent shadow-card transition-all duration-300 placeholder:text-muted-foreground/60"
      ...
      <div className="mt-6 text-center">
        <p className="text-xs text-muted-foreground mb-2">특허번호 예시</p>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {exampleKeywords.map((keyword) => (
            <button
              ...
              className="px-3 py-1.5 text-sm rounded-full bg-accent/10 hover:bg-accent/20 text-accent hover:text-accent transition-colors"
            >
              {keyword}
            </button>
          ))}
