    ...
    <div className="w-full max-w-2xl mx-auto animate-fade-up">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <Input
            type="text"
            placeholder="특허 등록번호를 입력하세요 (예: 10-1234567)"
            value={patentNumber}
      ...
      <div className="mt-6 text-center">
        <p className="text-sm text-muted-foreground mb-2">예시 번호로 시도해보세요</p>
        <div className="flex flex-wrap justify-center gap-2">
          {examplePatents.map((patent) => (
            <button
              key={patent}
              type="button"
              onClick={() => setPatentNumber(patent)}
              className="px-3 py-1.5 text-sm bg-secondary hover:bg-secondary/80 text-secondary-foreground rounded-full transition-colors duration-200"
              disabled={isLoading}
            >
              {patent}
            </button>
