...
export function TrlChart({ estimatedTrl, trlConfig }: TrlChartProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Enhanced TRL Stage Cards with icons and glassmorphism */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { 
            idx: 0, 
            check: estimatedTrl <= 3, 
            gradient: "from-red-500/10 to-orange-500/5",
            activeGradient: "from-red-500/20 to-orange-500/10",
            borderColor: "border-red-200/50 dark:border-red-800/30",
            activeBorder: "border-red-300 dark:border-red-700/50",
          ...
          { 
            idx: 1, 
            check: estimatedTrl >= 4 && estimatedTrl <= 6, 
            gradient: "from-amber-500/10 to-yellow-500/5",
            activeGradient: "from-amber-500/20 to-yellow-500/10",
        ...
        ].map(({ idx, check, gradient, activeGradient, borderColor, activeBorder, iconBg, iconColor, textColor }) => {
          const Icon = stageIcons[idx];
          return (
            <div
              key={idx}
              className={`relative p-3.5 rounded-2xl border backdrop-blur-sm transition-all duration-300 overflow-hidden group ${
                check
                  ? `bg-gradient-to-br ${activeGradient} ${activeBorder} shadow-sm`
                  : `bg-gradient-to-br ${gradient} ${borderColor} opacity-60 hover:opacity-80`
              }`}
            >
              {/* Glossy overlay */}
              ...
              <div className="relative flex flex-col items-center text-center gap-2">
                {/* Icon */}
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${check ? iconBg : 'bg-muted/50'} transition-all duration-300 ${check ? '' : 'grayscale'}`}>
                  <Icon className={`w-4 h-4 ${check ? iconColor : 'text-muted-foreground/50'}`} />
                </div>
                
                {/* Text */}
                <div>
                  <p className={`text-xs font-bold ${check ? textColor : 'text-muted-foreground/60'}`}>
                    {stages[idx]?.name || ["기초연구", "개발/실증", "상용화"][idx]}
                  </p>
                  <p className={`text-[10px] mt-0.5 ${check ? 'opacity-70' : 'opacity-50'} ${check ? textColor : 'text-muted-foreground/60'}`}>
                    {stages[idx]?.range || ["TRL 1-3", "TRL 4-6", "TRL 7-9"][idx]}
                  </p>
                </div>
                
                {/* Active indicator */}
                {check && (
                  <div className={`absolute -bottom-px left-1/2 -translate-x-1/2 w-12 h-0.5 rounded-full ${iconBg.replace('bg-', 'bg-gradient-to-r from-transparent via-').replace('/30', '/50')} to-transparent`} />
                )}
              </div>
            </div>
          );
