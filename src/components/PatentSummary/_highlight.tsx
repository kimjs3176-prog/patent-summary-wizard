...
export const HL_PATTERNS: { type: HLType; regex: RegExp }[] = [
  ...
  // 의미 있는 "(선행명사 )X을/를 + 동사" 구. 띄어쓰기로 분절된 선행 수식어를 포함해 통째로 강조.
  { type: "solution",    regex: /(?:^|[\s,.;:()「『"])((?:[가-힣]{1,8}\s+)?[가-힣]{2,8}(?:을|를)\s*(?:해결|극복|개선|달성|확보|구현|실현|돌파|상용화|국산화|대체|강화|향상|극대화|최적화|증대|확대|단축|절감))/g },
  ...
  { type: "concept",     regex: /([가-힣A-Za-z0-9]{2,10}(?:\s+[가-힣A-Za-z0-9]{2,10}){1,3}\s+(?:위험|한계|구조|모델|체계|시장|생태계|소비\s*구조|수익\s*구조))/g },
];
