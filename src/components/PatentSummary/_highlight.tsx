...
export const HL_PATTERNS: { type: HLType; regex: RegExp }[] = [
  { type: "quote",       regex: /([「『"][^「『"\n]{1,30}[」』"])/g },
  // [기준 시점 + 수치 + 단위] 묶음 강조 — "2026년까지 약 4억 5,000만 달러", "2026년 기준 국내 시장은 약 1,324억 원"
  // 정보의 최소 단위가 분절되지 않도록 시점 표현을 통째로 포함.
  { type: "money",       regex: /(\d{4}년(?:까지|\s*기준|\s*말|\s*초|\s*현재)?(?:\s*[가-힣A-Za-z]{1,10}(?:은|는|이|가|시장은|시장이|규모는|규모가)?)?\s*(?:약\s*)?\d[\d,]*(?:\.\d+)?\s*(?:조|억|만|천)?\s*(?:원|달러|USD|유로|EUR|위안|엔|파운드|KRW))/g },
  { type: "metric",      regex: /(\d{4}년(?:까지|\s*기준|\s*말|\s*초|\s*현재)?\s*(?:약\s*)?\d+(?:\.\d+)?\s?(?:%|%p|배))/g },
...
const FILLER_RE = /^(?:현재\s*)?(?:본|이|그|해당|동|주된|주요|일반|단순)?\s*(?:기술|발명|특허|연구|논문|장치|시스템|모델|구조|방식|방법|단계|분야|영역|구성|형태|형식|내용|결과|효과|기능|성능|문제|상황|특징|부분|측면|점|것|경우|사항|관점|차원|요소)$/;
...
const LEADING_TRIM_RE = /^(?:현재|향후|기존|기존의|결론적으로|종합적으로|전반적으로|단기적으로|중기적으로|장기적으로|특히|또한|그리고|따라서|이는|이러한|이와\s*같이|한편|반면|또는|또|그러나|하지만|즉|만약|뿐만\s*아니라|있으며|있고|있어|이며|있으나|있지만|하며|되며|하여|되어|하고|되고|에서|에는|에도|에게|에서는|에서도|내에|내에서|동안|만큼|보다|처럼|부터|까지|마다|조차|로서|로써|이고|이나|이라|에|와|과|로|으로|의|은|는|이|가|을|를|빠르게|크게|널리|매우|특히)\s+/;
...
const TRAILING_VERB_TAIL_RE = /(?:[가-힣]{1,8})(?:하는|되는|하여|되어|하며|되며|하고|되고|함으로써|됨으로써|하면|되면|기반한|한정된|예측하는|기반으로|기반한다|것이며|것이다|것으로|가능하다|가능하며|될\s*것이며|될\s*것이다|할\s*수\s*있는|할\s*수\s*있다|되는\s*것|하는\s*것)$/;

// 종료 접속어 — 매치 끝이 접속어/연결사로 끝나면 폐기.
const TRAILING_CONJ_RE = /(?:\s|^)(?:및|또는|혹은|그리고|따라서|즉|등의|등을|등이|등은|등|뿐만\s*아니라|반면|한편)$/;
...
// 시작 지시관형사/디스코스 보강 — "이러한", "이와 같은", "본", "해당", "동" 단독 수식어로 시작.
const LEADING_DEMONSTRATIVE_RE = /^(?:이러한|그러한|저러한|이와\s*같은|그와\s*같은|본|해당|동)\s+/;
...
// 마지막 어절이 명사형으로 끝나는지 검사.
// - 명사형 접미사(기술/시스템/방법/방안/효과/성능/구조/모델/장치/소재/원료/공정/체계/전략/솔루션/시장/산업/분야/기반/키트/세트/기업/마커/세포/유전자/단백질/항체/추출물/필름/모듈/플랫폼/서비스/제품/공급처/영역/단계 등) 또는
// - 한글 받침 명사(종결 한 글자가 받침을 가진 한자어/고유명사 패턴), 영문 약어, 숫자 단위 종결 허용.
const NOUN_SUFFIX_RE = /(?:기술|기법|시스템|방법|방안|효과|성능|구조|모델|장치|소재|원료|공정|체계|전략|솔루션|시장|산업|분야|기반|키트|세트|기업|마커|세포|유전자|단백질|항체|추출물|필름|모듈|플랫폼|서비스|제품|공급처|영역|단계|단축|향상|확보|증대|개선|제고|상승|감소|절감|단축|예방|치료|진단|관리|개발|적용|도입|확립|구현|실현|상용화|국산화|대체|강화|극대화|최적화|증대|확대|점유|차지|모니터링|예찰|예측|선발|판별|검출|회수율|생산성|수율|안정성|신뢰성|완성도|편의성|정확도|민감도|특이도|재현성|적용성|효율|시간|기간|위험|한계|문제|어려움|단점|취약점|결함|리스크|연구소|연구원|병원|학교|대학|공장|단지|시설|네트워크|데이터(?:베이스)?|알고리즘|프로토콜|프라이머|시퀀스|서열번호|품종|개체|샘플|kg|g|mg|t|톤|mm|cm|km|ml|L|°C|℃|%|%p|배|건|년|개월|점|명|곳|종|개)$/;

function endsWithNounLike(phrase: string): boolean {
  const last = phrase.split(/\s+/).pop() || "";
  if (!last) return false;
  // 영문 약어
  if (/^[A-Z0-9][A-Z0-9\-\/]*$/.test(last)) return true;
  // 숫자/단위 종결
  if (/\d[가-힣A-Za-z%]*$/.test(last)) return true;
  // 명사형 접미사
  if (NOUN_SUFFIX_RE.test(phrase)) return true;
  // 동사 활용 어미로 끝나면 명사형 아님
  if (/(?:하다|되다|하는|되는|하여|되어|하며|되며|하고|되고|한|된|할|될|함|됨|기|음)$/.test(last)) return false;
  // 한글 마지막 글자가 받침을 갖는 명사형(휴리스틱) — 2글자 이상 한글 어절이면 허용
  if (/^[가-힣]{2,}$/.test(last)) return true;
  return false;
}
...
function isValidHighlightPhrase(phrase: string): boolean {
  ...
  if (LEADING_DEMONSTRATIVE_RE.test(p + " ")) return false;
  if (TRAILING_PARTICLE_RE.test(p)) return false;
  if (TRAILING_CONJ_RE.test(p)) return false;
  // (b) 동사 활용형으로 종료
  ...
  // (e) 마지막 어절이 명사형(완전한 의미 단위)으로 끝나야 함
  if (!endsWithNounLike(p)) return false;
  return true;
}
