...
const HL_PATTERNS: { type: HLType; regex: RegExp }[] = [
  ...
  // 4) 단순 수치+단위 (CAGR/점유율 등) — 단독으로 의미가 약한 단위("차", "년", "회") 제외
  //    "6차" 단독 강조처럼 의미 없는 하이라이트를 방지하기 위해 단위를 핵심적인 것만 남긴다.
  ...
  // 7) 결과형 형용어가 붙은 핵심 명사구 — "극대화된 기술", "강화된 시스템" 등
...
function extractKeywordsFromPatent(
  ...
  const subjectPatterns: [RegExp, string][] = [
    // 한약재·약용식물 (제목에 자주 등장하는 한국 식물명 — 최우선 매칭)
    ...
    // 일반 생체분자 — 너무 흔하므로 "주성분/유효성분/고함량" 등 핵심성분 맥락에서만 매칭
  ...
  // 제목에서 구체적 소재를 찾았으면 본문 폴백 생략 (단백질 등 일반 분자가 잘못 끼어드는 것 방지)
