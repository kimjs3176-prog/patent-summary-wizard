...
function SectionTitle({ children, kicker, index }: { children: React.ReactNode; kicker?: string; index?: string }) {
  return (
    <div className="report-section-title mb-4">
      <div className="flex items-center gap-2.5 mb-2">
        <span className="report-section-rule" aria-hidden="true" />
        {index && (
          <span className="report-section-index font-mono text-[10px] font-bold tabular-nums">
            {index}
          </span>
        )}
        {kicker && <span className="bp-kicker">{kicker}</span>}
      </div>
      <h2 className="text-[20px] sm:text-[22px] font-bold text-[#191F28] leading-[1.35]">
        {children}
      </h2>
    </div>
  );
...
function TossPatentSummaryInner({
  ...
  return (
    <div className="text-[#191F28]" style={{ fontFamily: "'Pretendard','Inter',sans-serif" }}>
      ...
      <div ref={(node) => { aiBodyRef.current = node; printRef.current = node; }} data-toss-summary data-toss-surface className="bp-doc precision-report border border-border overflow-hidden max-w-[864px] mx-auto">
        ...
        <div className="px-5 sm:px-7 pb-12">
          ...
          <section className="report-hero relative -mx-5 sm:-mx-7 px-5 sm:px-7 pt-7 pb-6 mb-8">
            ...
            <p className="font-mono text-[12px] font-medium mb-5 tabular-nums">
          ...
          {patentData && (
            <section id="sec-basic" data-summary-section data-summary-label="기본 정보" className="report-section mb-8 scroll-mt-24">
          ...
          {/* 종합점수 + 세부점수 + TRL 통합 카드 */}
          <section id="sec-score" data-summary-section data-summary-label="사업화 점수" className="report-section mb-8 scroll-mt-24">
          ...
          {/* 도면 */}
          {drawings.length > 0 && (
            <section id="sec-figures" data-summary-section data-summary-label="특허 도면" className="report-section mb-8 scroll-mt-24">
          ...
          {isStreaming && sections.length === 0 ? (
            ...
            sections.map((sec, idx) => {
              ...
              return (
                <section key={idx} id={`sec-ai-${idx}`} data-summary-section data-summary-label={heading} className="report-section report-prose-section mb-8 scroll-mt-24">
          ...
          {/* AI 종합 의견 */}
          {details?.analysis && (
            <section id="sec-analysis" data-summary-section data-summary-label="AI 종합 분석" className="report-section mb-8 scroll-mt-24">
              <SectionTitle index="A1" kicker="AI 종합">AI 종합 분석 의견</SectionTitle>
              <div className="report-analysis-panel p-5 sm:p-6">
          ...
          {/* 관련 특허 — 기능·특징이 유사한 특허 */}
          {patentData && (
            <section id="sec-related" data-summary-section data-summary-label="관련 특허" className="report-section mb-8 scroll-mt-24">
          ...
          {patentData && !isStreaming && (
            <section id="sec-regulation" data-summary-section data-summary-label="예상 규제" className="report-section mb-8 scroll-mt-24">
