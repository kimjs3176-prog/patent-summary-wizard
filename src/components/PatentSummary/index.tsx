              ...
              {(() => {
                ...
                if (patentData.classifications && patentData.classifications.length > 0) {
                  const ipcBusinessMap: Record<string, string> = {
                    ...
                    'A23P': '식품성형', 'A22C': '식육가공', 'A22B': '도축기술',
                    'A01G': '스마트팜', 'A01H': '품종개량', 'A01K': '스마트축산',
                    'A01N': '친환경농약', 'A01C': '정밀파종', 'A01D': '수확자동화', 'A01J': '유가공',
                    'A01F': '수확후처리',
                    ...
                    'G16B': '바이오인포매틱스', 'G05B': '자동제어',
                    // H - 전기·통신
                    'H04L': 'IoT통신', 'H04W': '무선네트워크',
                    // F - 기계
                    'F26B': '건조장치', 'F25D': '냉장냉동',
                    // 3자리 매핑
                    'A23': '식품산업', 'A01': '농업기술', 'A22': '축산가공', 'A61': '헬스케어',
                    'C12': '바이오산업', 'C07': '의약화학', 'C08': '소재산업',
                    'G06': 'ICT융합', 'B01': '화학공정', 'H04': 'IoT', 'G01': '센싱기술',
                    'B65': '스마트물류', 'B02': '곡물가공', 'F26': '건조기술',
                  };
                ...
                if (patentData.titleKo) {
                  ...
                  // 대표 소재/대상 추출 — 특허 제목에서 핵심 소재를 키워드로 추가
                  const subjectPatterns: [RegExp, string][] = [
                    // 곡물·가루
                    [/밀가루/, '밀가루'], [/메밀|메밀가루/, '메밀'], [/쌀|미곡|현미/, '쌀'],
                    [/보리/, '보리'], [/옥수수/, '옥수수'], [/귀리|오트/, '귀리'], [/수수/, '수수'],
                    // 두류·유지
                    [/콩|대두/, '콩'], [/녹두/, '녹두'], [/팥/, '팥'], [/참깨|들깨/, '참깨·들깨'],
                    // 과일
                    [/사과/, '사과'], [/배(?![수양관])/, '배'], [/포도/, '포도'], [/감귤|귤/, '감귤'],
                    [/딸기/, '딸기'], [/복숭아/, '복숭아'], [/블루베리/, '블루베리'], [/감/, '감'],
                    [/키위/, '키위'], [/자두|매실/, '매실'], [/수박/, '수박'], [/참외/, '참외'],
                    // 채소
                    [/고추/, '고추'], [/마늘/, '마늘'], [/양파/, '양파'], [/배추/, '배추'],
                    [/무(?!인|선)/, '무'], [/토마토/, '토마토'], [/감자/, '감자'], [/고구마/, '고구마'],
                    [/당근/, '당근'], [/시금치/, '시금치'], [/브로콜리/, '브로콜리'],
                    [/파프리카/, '파프리카'], [/인삼|홍삼/, '인삼'],
                    // 축산·수산
                    [/한우|소고기|쇠고기/, '한우'], [/돼지|돈육/, '돼지'], [/닭|가금/, '닭'],
                    [/계란|달걀/, '계란'], [/우유|원유/, '우유'], [/치즈/, '치즈'],
                    [/새우/, '새우'], [/굴/, '굴'], [/김|해조/, '해조류'], [/미역/, '미역'],
                    // 특용작물
                    [/버섯/, '버섯'], [/차(?:잎|나무)/, '차'], [/녹차/, '녹차'],
                    [/꿀|벌꿀|봉밀/, '꿀'], [/프로폴리스/, '프로폴리스'],
                    // 기타 소재
                    [/전분/, '전분'], [/섬유|셀룰로오스/, '셀룰로오스'], [/키토산/, '키토산'],
                    [/펙틴/, '펙틴'], [/단백질/, '단백질'], [/아미노산/, '아미노산'],
                    [/비타민/, '비타민'], [/폴리페놀/, '폴리페놀'], [/카테킨/, '카테킨'],
                    [/유산균|젖산균/, '유산균'], [/효모/, '효모'], [/곰팡이|균주/, '균주'],
                  ];
                  subjectPatterns.forEach(([pattern, label]) => {
                    if (pattern.test(title) && !keywords.includes(label)) keywords.unshift(label); // 소재를 앞에 배치
                  });
                  ...
                  const efficacyMap: [RegExp, string][] = [
                    ...
                    [/친환경|유기농|무농약/, '친환경농업'],
                    [/스마트|IoT|센서/, '스마트농업'], [/드론|무인/, '농업드론'],
                    [/수경|양액/, '스마트재배'], [/저장|보관|신선/, '신선유통'],
                    [/김치|발효채소/, '김치산업'], [/도계|도축|육류/, '축산가공'],
                    [/수산|어류|해조/, '수산가공'], [/버섯|균사/, '버섯재배'],
                    [/음료|주스/, '음료제조'], [/제과|제빵/, '베이커리'],
                    [/나노|마이크로/, '나노기술'], [/바이오|생물/, '바이오기술'],
                    [/에너지|태양|풍력/, '신재생에너지'], [/폐기물|재활용/, '자원순환'],
                    [/탈모기/, '가금처리'], [/세척/, '세척기술'], [/선별|분류/, '선별기술'],
                    [/포장|패키징/, '포장기술'], [/냉각|냉장|냉동/, '냉장냉동'],
                    [/건조/, '건조기술'], [/분쇄|파쇄/, '분쇄기술'],
                    [/로봇/, '로봇기술'], [/자동화|자동/, '자동화기술'],
                    [/측정|계측/, '측정기술'], [/모니터링/, '모니터링'],
                  ];
                  efficacyMap.forEach(([pattern, label]) => {
                    if (pattern.test(title) && !keywords.includes(label)) keywords.push(label);
                  });

                  // Fallback: extract contextual keywords from title only if < 3 keywords
                  if (keywords.length < 3) {
                    const titleFallbacks: [RegExp, string][] = [
                      [/조성물/, '조성물'], [/제조/, '제조방법'], [/장치/, '장치개발'], [/시스템/, '시스템'],
                      [/방법/, '공정기술'], [/추출/, '추출정제'], [/코팅/, '코팅기술'],
                      [/가공/, '가공기술'], [/배양/, '배양기술'], [/정제/, '정제기술'],
                      [/진단/, '진단기술'], [/제어/, '제어기술'], [/분석/, '분석기술'],
                      [/검출/, '검출기술'], [/유전|게놈/, '유전공학'], [/효소/, '효소기술'],
                      [/종자|씨앗/, '종자개발'], [/살충|방제/, '병해충방제'],
                    ];
                    titleFallbacks.forEach(([pattern, label]) => {
                      if (keywords.length < 5 && pattern.test(title) && !keywords.includes(label)) keywords.push(label);
                    });
                  }
                }
