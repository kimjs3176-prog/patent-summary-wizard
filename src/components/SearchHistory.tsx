        ...
        {history.slice(0, 6).map((item) => {
          ...
          return (
            <div
              ...
              className="group inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary/40 border border-border/50 hover:border-primary/40 hover:bg-secondary/60 cursor-pointer transition-all duration-200 text-xs max-w-[280px]"
            ...
            >
              ...
              {shortTitle && (
                <span className="text-muted-foreground/70 group-hover:text-foreground/80 transition-colors truncate">
                  {shortTitle}
                </span>
              )}
              <button
                className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-primary ml-0.5 flex-shrink-0"
                onClick={(e) => {
                  e.stopPropagation();
