...
export function NoticeSection({ compact = false }: NoticeSectionProps = {}) {
    ...
    return (
      <div className="w-full">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-[13px] font-bold text-foreground">공지사항</h3>
        </div>
        <div className="flex flex-col gap-1">
          {notices.map((notice) => (
            <div key={notice.id}>
              <button
                className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-left hover:bg-muted/40 transition-colors"
                onClick={() => setExpandedId(expandedId === notice.id ? null : notice.id)}
              >
                {notice.is_pinned && <Pin className="w-3 h-3 text-primary flex-shrink-0" />}
                {importanceBadge(notice.importance)}
                <span className="text-[12px] font-medium text-foreground/85 flex-1 truncate">{notice.title}</span>
                {notice.content && (
              ...
              {expandedId === notice.id && notice.content && (
                <div className="px-2 pb-2 text-[12px] text-muted-foreground leading-[1.7] whitespace-pre-wrap">
                  {notice.content}
                </div>
              )}
            </div>
