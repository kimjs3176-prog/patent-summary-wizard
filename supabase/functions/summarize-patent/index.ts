...
serve(async (req) => {
  ...
  const limited = await enforceRateLimit(req, { bucket: "summarize-patent", limit: 20, windowSeconds: 300 });
  ...
  try {
    ...
    const promptVersion = "v16-richer-commercialization";
...
## 농산업활용 가능성 — 청구항·기술원리에서 직접 도출되는 적용 분야 → 현장 시나리오(누가/어디서/어떤 문제를 어떻게) → 기대 효과(수율·품질·인건비·에너지·로스율 등 정량 지표)를 연결어구로 자연스럽게 잇는다. 기술 원리와 무관한 비약(토양센서를 의료·우주에 적용 등) 금지. 확장 시 "기술원리가 동일하게 적용 가능한 인접 분야"임을 명시.
 ## 상용화전망 — 이 섹션은 다른 섹션보다 짧아서는 안 되며 최소 8문장(약 600~800자)으로 작성한다. 다음 7요소를 모두 포함: (1) 기술완성도(실험실/파일럿/실증/양산준비)와 남은 핵심 과제 1개 이상, (2) 단기(1~2년)·중기(3~5년) 상용화 경로·사업화 단계(라이선싱·기술이전·자체양산·합작), (3) 1차 수요처와 수익모델(B2B·로열티·OEM·구독), (4) 양산·스케일업 시 예상되는 기술적·공정적 제약과 원가 요인(설비·원료·수율·인증 비용 등) 1개 이상, (5) 인허가·표준·규격 등 제도적 진입 요건(식품·사료·농약·비료·기계 안전 등 해당 분야 기준) 1개 이상, (6) 경쟁·대체 기술 대비 우위 요소와 열위 요소를 각각 구체적으로 대비, (7) 마지막 1~2문장은 "본 기술은 ~한 강점과 ~한 한계를 동시에 가지며, ~ 조건이 충족될 경우 ~ 영역에서 우선 상용화가 유망하다" 형태로 강점·한계·성공조건·유망 적용영역을 모두 포함. 가능하면 정량 지표(예상 원가 절감률·수율·초기 투자 규모·도입 대상 규모)를 1개 이상 포함한다. TRL 숫자 직접 언급 금지, "발전 가능성이 크다"식 막연한 마무리 금지.
  · 특허 존속기간·잔여기간·보호기간·권리 수명·출원 후 경과연수·만료 시점은 상용화 근거 또는 한계로 절대 언급하지 않는다. 출원일이 입력에 있더라도 기간을 계산하거나 평가하지 않는다. 기간 서술을 뺀 만큼 기술완성도·양산 제약·인허가 요건·경쟁 비교 내용을 더 구체적으로 채워 분량을 유지한다.
