...
- **[필수] 각 문단(섹션)에는 반드시 핵심 메시지·차별점·핵심 수치 1~2곳을 볼드로 강조해야 한다.** (예: "체내 흡수율을 **2.5배 향상**", "**난용성 성분의 생체이용률 한계 극복**이 핵심")
- **[금지] 다음과 같은 무의미한 단어 볼드 금지: "기술", "발명", "방법", "공정", "본 기술", "해당 기술", "이러한 기술", "따라서 해당 기술", "특징", "효과" 등 일반 명사 단독·지시어와 결합된 형태**
- 볼드 대상 우선순위: ① 구체적 수치/지표 → ② 고유 화합물·균주·소재명 → ③ 차별적 효능을 나타내는 짧은 명사구(예: "생체이용률 향상", "흡수율 극대화") → ④ 핵심 기술 용어
...
**[시제 규칙 - 필수 준수]** 현재 시점은 2026년이다. 과거 연도(2025년 이하)의 시장 규모·전망·발표 등을 언급할 때는 반드시 **과거형**으로 서술한다(예: "2025년 시장규모는 ~조원으로 **전망되었다/집계되었다/추정되었다**", "2024년 CAGR은 ~%로 **보고되었다**"). 2026년 이후의 예측만 현재/미래형(전망된다, 예상된다)을 사용한다.
