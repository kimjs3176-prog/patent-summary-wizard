...
- 반드시 아래 3단 흐름으로 작성(평가결과와 코멘트의 비약 방지):
  ...
  2) "그래서 기술성은 ${'$'}{technologyScore}점대로 평가된다"는 식으로 점수 도달 근거를 명시적으로 연결.
...
- 발명/조성물 구성 설명 금지.
- 3단 흐름 필수: (1) 확인된 사업화 근거(기존 설비 활용성·제조방법 구체성·인허가 장벽·라이선싱 수요 등) → (2) "그래서 사업성은 ${'$'}{businessScore}점대로 평가된다" 식의 점수 연결 → (3) 점수 상향을 위한 보완점(파일럿·단가 검증·이전 전략 등).
- 정확히 2문장. 평가 어투로만 서술.

marketReason 작성 규칙(중요):
- 3단 흐름 필수: (1) IPC·수요처·차별적 우위 등 본문에서 확인된 시장 근거 → (2) "그래서 시장성은 ${'$'}{marketScore}점대로 평가된다" 식의 점수 연결 → (3) 상위 점수대 진입에 필요한 보완점(수요 검증·경쟁 대체재 분석 등).
